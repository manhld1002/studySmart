...
)
{
    ...
    var selectedSubject by remember {
        mutableStateOf("No subject selected")
    }
    ...
    DeleteDialog(
        ...
        bodyText = "Are you sure, you want to delete this session ? " + "This action can not be undo",
        onDismissRequest = {
        ...
        LazyColumn(
            ...
            item {
                ButtonsSection(
                    ...
                    finishButtonClick = {
                        val duration =
                            timerService.duration.toLong(DurationUnit.SECONDS)
                        if (duration >= 36)
                        {
                            ServiceHelper.triggerForeGroundService(
                                context = context,
...
private fun TimerSection(
    modifier: Modifier, hours: String, minutes: String, seconds: String
)
{
    Box(
        ...
        Box(
            modifier = Modifier
                .size(250.dp)
                .border(
                    5.dp, MaterialTheme.colorScheme.surfaceVariant, CircleShape,
                )
        )
        Row {
